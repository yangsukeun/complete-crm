...
export async function POST(req: Request) {
  try {
    ...
    if (role !== "EXECUTIVE" && role !== "ADMIN") {
      return NextResponse.json({ error: "대표만 부서를 등록할 수 있습니다." }, { status: 403 });
    }
    let body: unknown;
    try {
      body = await req.json();
    ...
    if (existing) {
      return NextResponse.json({ error: "이미 같은 이름의 부서가 있습니다." }, { status: 400 });
    }
    ...
    return NextResponse.json(
      { error: "부서 등록에 실패했습니다.", details: process.env.NODE_ENV === "development" ? message : undefined },
      { status: 500 }
    );
  }
}
