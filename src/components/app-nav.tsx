...
export function AppNav() {
  ...
  const workLinks = workGroupLinks.filter((l) => !l.featureKey || can(l.featureKey));
  const hrLinks = hrGroupLinks.filter(
    (l) => (!l.featureKey || can(l.featureKey)) && (!l.companyOnly || isCompany)
  );
  const financeLinks = financeGroupLinks.filter((l) => !l.featureKey || can(l.featureKey));
              ...
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  className={cn(
                    ...
                    hrLinks.some((l) => pathname === l.href || pathname.startsWith(l.href + "/"))
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  className={cn(
                    "flex items-center gap-1.5 px-3 py-2 text-sm font-medium transition-all duration-200",
                    financeLinks.some((l) => pathname === l.href || pathname.startsWith(l.href + "/"))
      ...
      <div className="flex overflow-x-auto border-t border-gray-100 bg-white px-4 py-2 md:hidden">
        <div className="flex gap-1">
          {[...mainLinks, ...workLinks, ...hrLinks, ...financeLinks].map(({ href, label, icon: Icon }) => {
            const isActive = pathname === href || pathname.startsWith(href + "/");
            const isHr = hrLinks.some((l) => l.href === href);
            const isFinance = financeLinks.some((l) => l.href === href);
            return (
              <Link
                key={href}
                href={href}
